<template>
	<div class="table-container">
		...
		>
			...
			<el-table-column type="index" label="Index" width="60" v-if="config.isSerialNo" />
			<el-table-column
			...
			<el-table-column label="Operation" width="100" v-if="config.isOperate">
				<template v-slot="scope">
					<el-popconfirm title="Are you sure you want to delete it?" @confirm="onDelRow(scope.row)">
						<template #reference>
			...
			<template #empty>
				<el-empty description="No data for now" />
			</template>
		</el-table>
		<div class="table-footer mt15">
			...
			<div class="table-footer-tool">
				...
				>
					<template #reference>
						<SvgIcon name="iconfont icon-quanjushezhi_o" :size="22" title="Settings" />
					</template>
					<template #default>
						<div class="tool-box">
							...
							/>
							<el-checkbox v-model="getConfig.isSerialNo" class="ml12 mr1" label="Index" />
							<el-checkbox v-model="getConfig.isSelection" class="ml12 mr1" label="Multiple" />
						</div>
						<el-scrollbar>
...
// 导出
const onImportTable = () => {
	if (state.selectlist.length <= 0) return ElMessage.warning('Select the data you want to export first.');
