<template>
  <div>
    <el-row :gutter="20">
      ...
      <el-col :span="2" :offset="1">Front-end configuration</el-col>
      <el-col :span="3" >Operation</el-col>
    </el-row>
    <el-form ref="formRef" :model="formData" label-width="0px" label-position="left" style="margin-top: 20px">
      ...
      >
        <el-col :span="4">
          <el-input v-if="item.edit" v-model="item.title" style="display: inline-block; width: 200px" placeholder="Please enter variable title"></el-input>
        ...
        <el-col :span="10">
          ...
          <div v-else-if="['img', 'imgs'].indexOf(item.form_item_type_label) > -1" :key="index + 7">
            ...
            >
              ...
              <div slot="tip" class="el-upload__tip">Please select an image and only upload jpg/png files.</div>
            </el-upload>
              ...
              <vxe-column title="operation" width="100" show-overflow>
                <template #default="{ row, index }">
                  <el-popover placement="top" width="160" v-model="childRemoveVisible">
                    <p>You can't recover from a deletion. Are you sure you want to delete it?</p>
                    <div style="text-align: right; margin: 0">
                      <el-button size="mini" type="text" @click="childRemoveVisible = false">Cancel</el-button>
                      <el-button type="primary" size="mini" @click="onRemoveChild(row, index, item.key)">Determine</el-button>
                    </div>
      ...
      <el-form-item>
        <el-button type="primary" @click="onSubmit(formRef)">Determine</el-button>
      </el-form-item>
    </el-form>
  </div>
</template>
        ...
        if (['img', 'imgs'].indexOf(item.form_item_type_label) > -1) {
          for (const arr of item.rule) {
            if (arr.required && item.value === null) {
              errorMessage(item.title + ' cannot be null');
              return;
            }
          }
        }
...
const handleUploadSuccess = (response: any, file: any, fileList: any, imgKey: any) => {
  ...
  if (code === 2000) {
    ...
    if (!type) {
      errorMessage('Only images are allowed to upload.');
    } else {
    ...
    errorMessage('Upload failure,' + JSON.stringify(msg));
  }
};
...
// 上传超出限制
const handleExceed = () => {
  errorMessage('Exceeds the number of file uploads.');
};
