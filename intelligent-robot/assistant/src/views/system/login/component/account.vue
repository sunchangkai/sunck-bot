...
export default defineComponent({
	...
	setup() {
		...
		const rules = reactive<FormRules>({
			username: [
				{ required: true, message: 'Please fill in the account number.', trigger: 'blur' },
			],
			password: [
				{
					required: true,
				...
				if (valid) {
					loginApi.login({ ...state.ruleForm, password: Md5.hashStr(state.ruleForm.password) }).then((res: any) => {
						if (res.code === 2000) {
							...
							// Session.set('session_id', res.data.session_id);
							// Session.set('session_name', res.data.session_name);
							Session.set('session_id', 1);
							Session.set('session_name', "test session");
